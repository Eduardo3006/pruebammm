<?php
    ...
    if(isset($_SESSION['user_login'])){  
        ...
        if($idAsignatura){
            ...
            $asignaturaActual = $stmt->fetch(PDO::FETCH_ASSOC);
        }
        elseif($idCarrera){
            $stmt = $pdo->prepare("SELECT * FROM asignatura WHERE idcarrera = :id LIMIT 1");
            $stmt->execute([':id'=>$idCarrera]);
            $asignaturaActual = $stmt->fetch(PDO::FETCH_ASSOC);
        }
        else{
...
.left::-webkit-scrollbar-thumb{
    ...
    <body>
        ...
        <?php
           $idCarreraLista = $asignaturaActual['idcarrera'] ?? null;
            ...
            if($row['role'] == 'estudiante'){
                ...
                $lista = $pdo->prepare("
                    SELECT 
                        ...
                        a.tema,
                        c.title AS carrera,
                        COUNT(act.idactividad) AS total_actividades
                            FROM asignatura a
                            INNER JOIN inscripciones i ON a.idasignatura = i.idasignatura
                            INNER JOIN carrera c ON a.idcarrera = c.id
                            LEFT JOIN actividades act ON act.idasignatura = a.idasignatura
                            WHERE i.idusuario = :uid
                            GROUP BY a.idasignatura
                        ");

                    $lista->execute([':uid'=>$id]);
                ...
                $lista->execute([':carrera'=>$idCarreraLista]);
            }
        ?>
    ...
    <ul class="menu-temas">
        <?php foreach($lista as $index => $item): ?>
            <li class="tema-card <?= $item['idasignatura']==$idAsignatura ? 'active':'' ?>">
                ...
                <a href="actividades.php?idasignatura=<?= $item['idasignatura'] ?>">
                    ...
                    <div class="tema-bottom">
                        <span class="materia">
                            <?= htmlspecialchars($item['clave'] ?? 'Materia') ?>
                        </span>
        ...
        <?php endforeach; ?>
                ...
                <div class="right-content">
                    ...
                    <div class="header-top">
                        <h2><?= htmlspecialchars($asignaturaActual['tema'] ?? 'Sin asignatura') ?></h2>
                    </div>

                    <div class="card-section">
                        <div class="section-header" onclick="irContenido(<?= $idAsignatura ?>)">
