    ...
    <body>
        ...
        <main class="main">
            <?php
                ...
                if(!$idasignatura){
                    ...
                    <div class="card-empty">
                        ...
                        <h2>No seleccionaste una asignatura</h2>
                    </div>';
                    exit();
                }
                $listas = $pdo->prepare("SELECT * FROM actividades WHERE idasignatura = :carrera");
                $listas->execute([':carrera'=>$idasignatura]);

                $resultados = $listas->fetchAll(PDO::FETCH_ASSOC);
            ...
            <?php if(empty($resultados)): ?>
                <div class="card-empty">
                    ...
                    <p>Actualmente esta asignatura no tiene actividades asignadas.</p>
                    
                    <button onclick="abrirModalAsignatura()" class="btn-primary">
                        📚 Ver otra asignatura
                    </button>
                </div>
            <?php else: ?>
                <?php foreach($resultados as $items): ?>
                    <div class="card-section">
                        <div class="section-header"> 
                            <h3><?= htmlspecialchars($items['titulo']) ?></h3>

                            <p><?= htmlspecialchars($items['descripcion']) ?></p>

                            <small>📅 Inicio: <?= $items['fecha_inicio'] ?? 'N/A' ?></small><br>
                            <small>⏳ Entrega: <?= $items['fecha_entrega'] ?? 'N/A' ?></small><br>

                            <span class="badge"><?= $items['tipo'] ?></span>
                            <span class="estado"><?= $items['estado'] ?></span>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
            ...
            <div class="modal-content">
                ...
                <form method="GET" action="actividades.php"> 
                    ...
                    <select name="idasignatura" required>
                        ...
                        <?php
                            if($row['role'] == 'admin' || $row['role'] == 'profesor'){
                                // 🔓 Ver TODAS las asignaturas
                                $asigModal = $pdo->prepare("SELECT idasignatura, tema FROM asignatura");
                                $asigModal->execute();
                            } else {
                                // 🔒 Solo asignaturas inscritas
        ...
        <script>
            function irAsignatura(id){
                window.location = "lista_actividades.php?idasignatura=" + id;
            }
        </script>
